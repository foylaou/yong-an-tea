...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: coupon } = await db.from('coupons').select('*').eq('id', id).single();
  if (!coupon) {
  ...
  const { data: members } = await db.from('profiles').select('line_user_id').not('line_user_id', 'is', null);
  const userIds = (members ?? []).map((m) => m.line_user_id as string);
  if (!userIds.length) {
    return NextResponse.json({ error: '目前沒有已綁定 LINE 的會員' }, { status: 400 });
  }
  ...
  for (let i = 0; i < userIds.length; i += CHUNK_SIZE) {
    ...
    try {
      await multicastMessage(chunk, [{ type: 'text', text }], botSettings.channelAccessToken);
      sent += chunk.length;
    } catch (err) {
