...
export async function POST(request: NextRequest) {
  ...
  const [{ data: customer }, { data: coupon }] = await Promise.all([
    db.from('customers').select('id, name, line_user_id').eq('id', customerId).single(),
  ...
  if (!customer.line_user_id) {
    return NextResponse.json({ error: '此會員尚未綁定 LINE 帳號，無法發送' }, { status: 400 });
  }
  if (!coupon) {
  ...
  try {
    await pushMessage(customer.line_user_id, [{ type: 'text', text }], botSettings.channelAccessToken);
  } catch (err) {
