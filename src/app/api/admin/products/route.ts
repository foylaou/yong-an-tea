...
export async function GET(request: NextRequest) {
    ...
    if (search) {
        query = query.or(
            `title.ilike.%${search}%,sku.eq.${parseInt(search, 10) || 0}`
        );
    }
...
export async function POST(request: NextRequest) {
    ...
    if (galleryImages && galleryImages.length > 0) {
        ...
        if (!productData.home_collection_img)
            productData.home_collection_img = first.md_url;
        if (!productData.category_banner_img)
            productData.category_banner_img = first.md_url;
    }
