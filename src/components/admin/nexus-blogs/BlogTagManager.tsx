    ...
    <div>
      ...
      <form onSubmit={handleAdd} className="mb-4 flex flex-wrap items-end gap-3">
        <fieldset className="fieldset">
          ...
          <input type="text" value={newName} onChange={(e) => handleNewNameChange(e.target.value)} placeholder="標籤名稱" className="input input-sm w-48" />
        </fieldset>
        <fieldset className="fieldset">
          <legend className="fieldset-legend">Slug</legend>
          <input type="text" value={newSlug} onChange={(e) => setNewSlug(e.target.value)} placeholder="tag-slug" className="input input-sm w-48" />
        </fieldset>
              ...
              tags.map((tag) =>
                editId === tag.id ? (
                  ...
                  <tr key={tag.id}>
                    ...
                    <td className="text-base-content/60">{tag.slug}</td>
                    <td>
                      <div className="flex gap-2">
                        <button onClick={() => startEdit(tag)} className="btn btn-xs btn-outline btn-info">
                          編輯
                        </button>
                        <button onClick={() => setDeleteTarget(tag)} className="btn btn-xs btn-outline btn-error">
                          刪除
                        </button>
                      </div>
                    </td>
