...
export function CouponTable({
    ...
    return (
        <div>
            ...
            {/* Desktop table */}
            <div
                className={`hidden overflow-x-auto md:block ${loading ? 'opacity-50' : ''}`}
            >
                <table className="table">
                    <thead>
                        <tr>
                            ...
                            <th>有效期間</th>
                            <th className="text-center">發放</th>
                            ...
                            coupons.map((coupon: any) => (
                                <tr key={coupon.id}>
                                    <td className="font-mono font-medium">
                                        {coupon.code}
                                        {coupon.description && (
                                            <div className="text-base-content/40 font-sans text-xs font-normal">
                                                {coupon.description}
                                            </div>
                                        )}
                                    </td>
                                    ...
                                    <td className="text-center">
                                        <span
                                            className={`badge badge-sm ${coupon.is_public ? 'badge-info' : 'badge-outline'}`}
                                        >
                                            {coupon.is_public
                                                ? '公開'
                                                : '限定發放'}
                                    ...
                                    <td className="text-center">
                                        <span
                                            className={`badge badge-sm ${coupon.is_active ? 'badge-success' : 'badge-ghost'}`}
                                        >
                                    ...
                                    <td className="text-center">
                                        <div className="flex items-center justify-center gap-2">
                                            ...
                                            <button
                                                ...
                                                className="link text-error text-sm"
                                            >
                                                {deleting === coupon.id
                                                    ? '刪除中...'
                                                    : '刪除'}
            ...
            <div
                className={`space-y-3 md:hidden ${loading ? 'opacity-50' : ''}`}
            >
                {coupons.length === 0 ? (
                        ...
                        <div key={coupon.id} className="card card-border">
                            <div className="card-body gap-2 p-4">
                                ...
                                <div className="text-sm">
                                    {discountTypeLabel[coupon.discount_type] ||
                                        coupon.discount_type}{' '}
                                    ·{' '}
                                    {formatDiscountValue(
                                        coupon.discount_type,
            ...
            <Pagination
                ...
                total={total}
                perPage={perPage}
                onPageChange={(p) => fetchCoupons({ page: p })}
            />
        </div>
    );
}
