...
export function ProductForm({ categories, initialData, isEdit = false }: ProductFormProps) {
  ...
  // --- 產品變體 (variants) ---
  type Variant = { name: string; price: number; discount_price: number | null; cost_price: number | null; stock_qty: number; sku: string; image_index: number | null };
  ...
  const addVariant = () =>
    setVariants([...variants, { name: '', price: 0, discount_price: null, cost_price: null, stock_qty: 0, sku: '', image_index: null }]);
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <fieldset className="fieldset">
              ...
              <input
                type="number"
                step="0.01"
                {...register('cost_price', { setValueAs: (v: string) => (v === '' ? null : Number(v)) })}
                className="input w-full"
              />
              {errors.cost_price && <p className="text-error mt-1 text-sm">{errors.cost_price.message}</p>}
            </fieldset>
            <fieldset className="fieldset">
          ...
          <div className="space-y-3">
            {variants.map((v, idx) => (
              <div key={idx} className="flex flex-wrap items-end gap-3">
                ...
                <div className="w-28">
                  {idx === 0 && <label className="text-base-content/50 mb-1 block text-xs font-medium">成本價</label>}
                  <input
                    type="number"
                    step="0.01"
                    value={v.cost_price ?? ''}
                    onChange={(e) => updateVariant(idx, 'cost_price', e.target.value === '' ? null : Number(e.target.value))}
                    className="input input-sm w-full"
                  />
                </div>
