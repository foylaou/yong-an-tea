...
import ProductImageGallery, {
    type GalleryImage,
} from '@/components/admin/common/ProductImageGallery';
...
export function ProductForm({
    ...
    const defaultValues: ProductFormData = {
        ...
        category_ids:
            initialData?.product_categories?.map((pc) => pc.category_id) || [],
    };
    ...
    const addAttribute = () =>
        syncAttrs([...attributes, { name: '', value: '', unit: '' }]);
    const removeAttribute = (idx: number) =>
        syncAttrs(attributes.filter((_, i) => i !== idx));
    const updateAttribute = (
        ...
        syncAttrs(
            attributes.map((a, i) => (i === idx ? { ...a, [field]: val } : a))
        );
    };
    ...
    const [variantStockMode, setVariantStockMode] = useState<
        'shared' | 'independent'
    ...
    const updateVariant = (
        ...
        setVariants(
            variants.map((v, i) => (i === idx ? { ...v, [field]: val } : v))
        );
    };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <fieldset className="fieldset">
                            ...
                            <input
                                {...register('slug')}
                                readOnly={isEdit}
                                className={`input w-full ${isEdit ? 'cursor-not-allowed opacity-60' : ''}`}
                            />
                            {errors.slug && (
                                <p className="text-error mt-1 text-sm">
                                    {errors.slug.message}
                        ...
                        <fieldset className="fieldset">
                            <legend className="fieldset-legend">成本價</legend>
                            <input
                                type="number"
                                step="0.01"
                                {...register('cost_price', {
                        ...
                        <div>
                            ...
                            {initialData?.puck_data != null && (
                                <p className="text-base-content/50 mt-2 text-xs">
                                    已有頁面內容（
                                    {(initialData.puck_data as any)?.content
                                        ?.length ?? 0}{' '}
                                    個區塊）
                                </p>
                            )}
                            ...
                            <div className="flex gap-4">
                                ...
                                <label className="flex cursor-pointer items-center gap-2 text-sm">
                                    <input
                                        ...
                                        onChange={() =>
                                            setVariantStockMode('independent')
                                        }
                                    />
                                    <span>各規格獨立庫存</span>
                                </label>
                        ...
                        {variants.map((v, idx) => (
                            ...
                            >
                                <div className="flex-1 basis-40">
                                    {idx === 0 && (
                                        <label className="text-base-content/50 mb-1 block text-xs font-medium">
                                            變體名稱 *
                                        </label>
                                    )}
                                    <input
            ...
            <div className="card card-border bg-base-100">
                <div className="card-body">
                    ...
                    <div className="border-base-300 mt-4 rounded-lg border p-4">
                        ...
                        {isEdit && watch('slug') && (
                            <div className="mt-3 flex items-center gap-2">
                                <button
                                    type="button"
                                    onClick={() => {
                                        const url = `${window.location.origin}/products/${watch('slug')}`;
                                        navigator.clipboard
                                            .writeText(url)
                                            .then(() => {
                                                setLinkCopied(true);
