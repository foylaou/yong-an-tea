...
function SortableProductRow({ product }: { product: AdminProduct }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="border-base-300 bg-base-100 flex items-center gap-3 rounded-box border px-3 py-2">
      <button type="button" className="cursor-grab touch-none text-base-content/40 hover:text-base-content" {...attributes} {...listeners}>
        <span className="iconify lucide--grip-vertical size-4.5" />
      </button>
      {product.xs_image ? (
        <CachedImage src={getImageUrl(product.xs_image)} alt={product.title} width={32} height={32} className="h-8 w-8 rounded object-cover" />
      ) : (
        <div className="bg-base-200 text-base-content/40 flex h-8 w-8 items-center justify-center rounded text-xs">N/A</div>
      )}
      <span className="flex-1 truncate text-sm font-medium">{product.title}</span>
      <span className="text-base-content/40 text-xs">#{product.sort_order ?? 0}</span>
    </div>
  );
}
...
function DndSortForm({
  ...
  return (
    <div>
      <p className="text-base-content/60 mb-3 text-sm">拖曳商品以調整排序，排在上方的商品會優先顯示。</p>
      ...
      <div className="mt-4 flex justify-end gap-3">
        ...
        <button type="button" onClick={() => onConfirm(items)} className="btn btn-sm btn-primary">
          儲存排序
        </button>
      </div>
    </div>
...
// ---------------------------------------------------------------------------
function SortableBestsellerRow({ product, onRemove }: { product: AdminProduct; onRemove: (id: string) => void }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: product.id });
  const style = { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.5 : 1 };
  ...
  return (
    <div ref={setNodeRef} style={style} className="border-base-300 bg-base-100 flex items-center gap-3 rounded-box border px-3 py-2">
      ...
      <span className="flex-1 truncate text-sm font-medium">{product.title}</span>
      <button type="button" onClick={() => onRemove(product.id)} className="text-base-content/40 hover:text-error">
...
function BestsellerForm({
  ...
  const [selectedProducts, setSelectedProducts] = useState<AdminProduct[]>(() => {
    ...
    return initialData.productIds.map((id) => byId.get(id)).filter((p): p is AdminProduct => Boolean(p));
  });
  ...
  return (
    <div>
      <fieldset className="fieldset mb-2">
        ...
        <select value={mode} onChange={(e) => setMode(e.target.value as 'auto' | 'custom')} className="select w-full">
      ...
      {mode === 'custom' && (
        <>
          <fieldset className="fieldset mb-1">
            ...
            <select value={addValue} onChange={(e) => handleAdd(e.target.value)} className="select w-full">
          ...
          <p className="text-base-content/60 mb-2 text-sm">拖曳調整順序，點擊 X 移除。前台首頁將依此順序顯示暢銷商品。</p>
          <div className="max-h-[40vh] space-y-1 overflow-y-auto">
            {selectedProducts.length === 0 ? (
              <p className="text-base-content/40 py-4 text-center text-sm">尚未選擇商品</p>
            ) : (
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      ...
      {mode === 'auto' && <p className="text-base-content/60 text-sm">系統將根據訂單銷量自動顯示最暢銷的商品。</p>}
...
function CategoryBadges({ product }: { product: AdminProduct }) {
  const cats = product.product_categories?.map((pc) => pc.categories?.name).filter(Boolean) as string[] | undefined;
...
export function ProductsTable({ initialProducts, initialTotal, initialPage, perPage, categories }: ProductsTableProps) {
  ...
  async function handleBestsellerSetting() {
    ...
    const result = await bestsellerDialog({
      cardTitle: '暢銷商品設定',
      cardStyle: 'rounded-box bg-base-100 border border-base-300 shadow-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-auto',
      renderForm: ({ onConfirm, onCancel }) => (
        <BestsellerForm
          initialData={{ mode: (bsSettings.bestseller_mode as 'auto' | 'custom') || 'auto', productIds: currentIds }}
  ...
  async function fetchProducts(params: { page?: number; search?: string; categoryId?: string; status?: string; sortBy?: SortMode }) {
    setLoading(true);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const url = deleteMode === 'hard' ? `/api/admin/products/${deleteTarget.id}?hard=true` : `/api/admin/products/${deleteTarget.id}`;
  ...
  async function handleSortChange(mode: SortMode) {
    ...
    if (mode === 'custom') {
      ...
      const result = await editDialog({
        cardTitle: '自訂商品排序',
        cardStyle: 'rounded-box bg-base-100 border border-base-300 shadow-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-auto',
        renderForm: ({ onConfirm, onCancel }) => <DndSortForm initialData={allProducts} onConfirm={onConfirm} onCancel={onCancel} />,
      });

      if (result) {
        const items = result.map((p, idx) => ({ id: p.id, sort_order: idx }));
  ...
  const columns = useMemo<ColumnDef<AdminProduct, any>[]>(
    () => [
      ...
      columnHelper.accessor('title', {
        header: '商品名稱',
        cell: ({ row }) => (
          <Link href={`/admin/products/${row.original.id}/edit`} className="link link-hover font-medium">
            {row.original.title}
          </Link>
        ),
      }),
      columnHelper.accessor('sku', { header: 'SKU', cell: ({ getValue }) => getValue() || '-' }),
      columnHelper.display({
        id: 'price',
        header: '價格',
        cell: ({ row }) =>
          row.original.discount_price ? (
            <span>
              <span className="text-error">${row.original.discount_price}</span>{' '}
              <span className="text-base-content/40 text-xs line-through">${row.original.price}</span>
            </span>
          ) : (
      ...
      columnHelper.accessor('is_active', {
        header: '狀態',
        cell: ({ getValue }) => <span className={`badge badge-sm ${getValue() ? 'badge-success' : 'badge-ghost'}`}>{getValue() ? '上架' : '下架'}</span>,
      }),
      columnHelper.display({
        id: 'actions',
        header: '操作',
        cell: ({ row }) => {
          ...
          return (
            <div className="flex gap-2">
              <Link href={`/admin/products/${product.id}/edit`} className="btn btn-xs btn-outline btn-info">
                編輯
              </Link>
              {product.is_active ? (
                <button onClick={() => openDeleteDialog(product, 'soft')} className="btn btn-xs btn-outline btn-warning">
                  下架
                </button>
              ) : (
                <button onClick={() => handleActivate(product)} className="btn btn-xs btn-outline btn-success">
                  上架
                </button>
              )}
              <button onClick={() => openDeleteDialog(product, 'hard')} className="btn btn-xs btn-outline btn-error">
                刪除
              </button>
            </div>
          );
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <button type="button" onClick={handleBestsellerSetting} className="btn btn-sm btn-warning">
          暢銷商品設定
        </button>
        ...
        <select value={sortMode} onChange={(e) => handleSortChange(e.target.value as SortMode)} className="select select-sm">
      ...
      <div className="space-y-3 md:hidden">
        {loading ? (
          ...
          <div className="text-base-content/40 py-8 text-center">沒有找到商品</div>
        ) : (
              ...
              <div className="card-body gap-2 p-4">
                <div className="flex items-start gap-3">
                  {product.xs_image ? (
                    ...
                    <div className="bg-base-200 text-base-content/40 flex h-12 w-12 shrink-0 items-center justify-center rounded text-xs">N/A</div>
                  )}
                  <div className="min-w-0 flex-1">
                    <Link href={`/admin/products/${product.id}/edit`} className="link link-hover font-medium">
                      {product.title}
                    </Link>
                    <div className="text-base-content/50 text-xs">SKU: {product.sku || '-'}</div>
                    <div className="mt-1 flex items-center gap-2">
                      {product.discount_price ? (
                        <>
                          <span className="text-error text-sm">${product.discount_price}</span>
                          <span className="text-base-content/40 text-xs line-through">${product.price}</span>
                        </>
                      ) : (
                        <span className="text-sm">${product.price}</span>
                      )}
                      <span className={`badge badge-xs ${product.is_active ? 'badge-success' : 'badge-ghost'}`}>
      ...
      <Pagination page={page} total={total} perPage={perPage} onPageChange={(p) => fetchProducts({ page: p })} />
