...
import {
  wishlistPageSettingsSchema,
  type WishlistPageSettingsData,
...
interface WishlistPageSettingsProps {
...
export default function WishlistPageSettings({ initialData }: WishlistPageSettingsProps) {
  // JSON field via state
  const [thItems, setThItems] = useState<ThItem[]>(() => parseJson(initialData, 'wishlist_th_list_json', []));
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<WishlistPageSettingsData>({
    resolver: zodResolver(wishlistPageSettingsSchema),
    defaultValues: {
      wishlist_th_list_json: (initialData.wishlist_th_list_json as string) || '',
      wishlist_clear_btn_text: (initialData.wishlist_clear_btn_text as string) || '',
    },
  });
  ...
  async function onSubmit(data: WishlistPageSettingsData) {
    setSubmitting(true);
    setServerError(null);
    setSuccess(false);
    ...
    try {
      const res = await fetch('/api/admin/settings', {
        ...
        body: JSON.stringify({
          ...
          settings: {
            ...data,
            wishlist_th_list_json: JSON.stringify(thItems),
          },
        }),
      });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <section className="rounded-lg bg-white p-6 shadow">
        <h2 className="mb-4 text-lg font-semibold text-gray-900">願望清單設定</h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">清空按鈕文字</label>
            <input type="text" {...register('wishlist_clear_btn_text')}
            ...
            {errors.wishlist_clear_btn_text && <p className="mt-1 text-sm text-red-600">{errors.wishlist_clear_btn_text.message}</p>}
          </div>
        </div>
      </section>
