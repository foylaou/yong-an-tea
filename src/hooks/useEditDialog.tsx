...
export const useEditDialog = <T = Record<string, unknown>>(): UseEditDialogReturn<T> => {
  ...
  const EditComponent = useMemo(() => {
    ...
    const modalClasses = [
      options.cardStyle || "rounded-lg bg-white border border-gray-200 shadow-lg max-w-2xl w-full mx-4 max-h-[80vh] overflow-auto",
      options.modalClassName || ''
    ].join(' ');

    // 動態設置 z-index + 背景色（inline 確保 Tailwind v4 相容）
