...
import {
    ...
    COUPON_QUERY_KEYWORDS,
} from './line-bot-keywords';
import { recordCouponRecipient } from './coupons-db';
...
/**
 * Anyone can ask, no LINE Login / member binding required, unlike order
 * status — but only lists *public* coupons (is_public = true). A 限定發放
 * coupon shouldn't be advertised to everyone who asks, even though the
 * keyword itself doesn't require being signed in — see coupons.is_public's
 * doc comment. Reply-based, same as order status, so this doesn't touch the
 * push quota either.
 */
export async function buildCouponReply(): Promise<LineMessage[]> {
    ...
    const { data: coupons } = await supabase
        .from('coupons')
        .select(
            'code, description, discount_type, discount_value, expires_at, starts_at'
        )
        .eq('is_active', true)
        .eq('is_public', true)
    ...
    if (!coupons?.length) {
        return [
            { type: 'text', text: '目前沒有進行中的優惠券活動，敬請期待！' },
        ];
    }
...
export async function sendWelcomeCoupons(lineUserId: string): Promise<void> {
    ...
    const { data: coupons } = await supabase
        .from('coupons')
        .select(
            'id, code, description, discount_type, discount_value, expires_at, starts_at'
        )
        .eq('is_active', true)
    ...
    // A welcome coupon defaults to 限定發放 (is_public stays false unless an
    // admin also broadcast it separately) — so redeeming it needs an explicit
    // coupon_recipients grant, same as any admin-issued coupon. That needs a
    // customers.id, which this brand-new member may not have yet; find or
    // create one here rather than skip the grant, or the coupon we're about
    // to hand them would be unredeemable. Mirrors /api/account/member-qr's
    // find-or-create.
    ...
    if (customerId) {
        await Promise.all(
            coupons.map((c) => recordCouponRecipient(c.id, customerId!))
        );
    }
 ...
 */
export async function buildOrderStatusReply(
    ...
    if (!profile) {
        return [
            {
                type: 'text',
                text: '尚未綁定會員帳號，請先至網站使用 LINE 登入後再查詢訂單。',
            },
        ];
    }
