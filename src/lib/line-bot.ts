...
export async function getLineBotSettings(): Promise<LineBotSettings> {
  ...
  const [{ data: secretRows, error: secretError }, { data: publicRows, error: publicError }] = await Promise.all([
    supabase.from('protected_settings').select('key, value').eq('group', 'line_bot'),
    supabase.from('site_settings').select('key, value').eq('group', 'line_bot_public'),
  ]);

  if (secretError || publicError || !secretRows || !publicRows) {
    throw new Error('無法讀取 LINE 官方帳號設定，請至後台系統設定 > LINE 官方帳號頁面填入');
  }
  ...
  const result: LineBotSettings = {
    ...
    channelAccessToken: String(settings.line_bot_channel_access_token ?? ''),
...
async function callMessagingApi(path: string, body: unknown, accessToken: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`LINE Messaging API ${path} failed: ${res.status} ${errText}`);
  }
}
 ...
 */
export async function replyMessage(replyToken: string, messages: LineMessage[], accessToken: string): Promise<void> {
  await callMessagingApi('/v2/bot/message/reply', { replyToken, messages }, accessToken);
}
 ...
 */
export async function pushMessage(to: string, messages: LineMessage[], accessToken: string): Promise<void> {
  await callMessagingApi('/v2/bot/message/push', { to, messages }, accessToken);
}
 ...
 */
export async function multicastMessage(to: string[], messages: LineMessage[], accessToken: string): Promise<void> {
  await callMessagingApi('/v2/bot/message/multicast', { to, messages }, accessToken);
}
