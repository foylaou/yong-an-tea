...
export const LIFF_FEATURES: LiffFeature[] = [
    ...
    { path: '/liff/my-coupons', label: '我的優惠券' },
...
export function resolveLiffAction(
    path: string,
    liffId: string
): RichMenuAction {
...
export function buildAreasFromTemplate(
    templateId: string,
        ...
        for (let col = 0; col < t.cols; col++) {
            ...
            areas.push({
                bounds: {
                    ...
                    width:
                        col === t.cols - 1
                            ? RICHMENU_SIZE.width - col * cellWidth
                            : cellWidth,
                    height:
                        row === t.rows - 1
                            ? RICHMENU_SIZE.height - row * cellHeight
                            : cellHeight,
                },
                action: actions[i],
            });
...
async function callLineApi(
    ...
    if (!res.ok) {
        ...
        throw new Error(
            `LINE Rich Menu API ${path} failed: ${res.status} ${body}`
        );
    }
    return res;
}
...
export async function listRichMenus(
    accessToken: string
): Promise<{ richMenuId: string; name: string }[]> {
    const res = await callLineApi(
 ...
 */
export async function upsertRichMenuAlias(
    aliasId: string,
    richMenuId: string,
    accessToken: string
): Promise<void> {
