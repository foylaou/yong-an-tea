...
export default function MyQrPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const dataUrl = await QRCode.toDataURL(encodeCustomerQr(data.id), { width: 320, margin: 2 });
  ...
  return (
    <div style={{ maxWidth: 400, margin: '0 auto', padding: '24px 16px', textAlign: 'center', fontFamily: 'sans-serif' }}>
      <h1 style={{ fontSize: 20, fontWeight: 600, marginBottom: 16 }}>我的會員條碼</h1>

      {(status === 'init' || status === 'loading') && <p>載入中...</p>}
      ...
      {status === 'ready' && (
        <div>
          ...
          {qrDataUrl && (
            ...
            <img src={qrDataUrl} alt="會員 QR code" style={{ width: '100%', maxWidth: 320, borderRadius: 8, border: '1px solid #eee' }} />
          )}
          <p style={{ marginTop: 16, fontSize: 13, color: '#666' }}>結帳時請出示這個畫面給店員掃描</p>
        </div>
      )}
